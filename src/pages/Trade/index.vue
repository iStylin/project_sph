  ...
  <div class="trade-container">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ defaultUserAddress.address }}</span>
        收货人：<span>{{ defaultUserAddress.name }}</span>
...
export default {
  ...
  computed: {
    ...
    // 总数量
    totalNum() {
      return this.$store.state.trade.tradeInfo.totalNum;
    },
    // 总额
    totalAmount() {
  ...
  methods: {
    ...
    // 提交订单
    async submitOrder() {
      let stradeNo = this.$store.state.trade.tradeInfo.tradeNo;
      let data = {
        consignee: this.defaultUserAddress.name,
        consigneeTel: this.defaultUserAddress.phoneNum,
        deliveryAddress: this.defaultUserAddress.address,
        paymentWay: "ONLINE",
        orderComment: this.msg,
        orderDetailList: this.detailArrayList,
      };
      let result = await this.$api.reqSubmitOrder(stradeNo, data);
      if(result.code == 200){
