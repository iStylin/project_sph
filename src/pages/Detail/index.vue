            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  autocomplete="off"
                  class="itxt"
                  @change="handlerSkuNum"
                  v-model="skuNum"
                />
                <a
                  href="javascript:"
                  class="plus"
                  @click="skuNum >= 9999 ? (skuNum = 9999) : skuNum++"
                  >+</a
                >
                <a
                  href="javascript:"
              ...
              <div class="add">
                <!-- 以前的路由跳转，从A路由跳转到B路由，此处加入购物车，进行路由跳转之前，需要发请求告知服务器加购的产品，
                服务器进行响应的存储
                 -->
...
export default {
  ...
  methods: {
    ...
    changeChecked(spuSaleAttrValue, spuSaleAttrValueList) {
      // spuSaleAttrValue -->当前点击的对象 ---子分类
      // spuSaleAttrValueList ---> 存储当前单击对象的容器 ---大分类
      spuSaleAttrValueList.forEach((element) => {
        // 置当前点击对象所属的大分类里所有子分类ischecked为0  ---》未选中状态（无高亮）
        element.isChecked = 0;
      });
      // 设置当前点击的子分类ischecked为1  ---》选中状态（高亮）
      spuSaleAttrValue.isChecked = 1;
    },
    // 用户加入购物车，自定义数量控制
    handlerSkuNum() {
      // 用户输入非法：【输入内容含有非数字，使用输入内容*1，检测】
    ...
    async addShopCart() {
      /*加入购物车：
        ...
        2、如果成功则进行路由跳转 --- 传递参数
      */
      let result = await this.$store.dispatch("addCartAndEditSkuNum", {
